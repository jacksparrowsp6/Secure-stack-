...
for (const file of files) {
  ...
  articles.push({ slug: meta.slug || file.replace(/\.md$/, ''), canonicalUrl: `https://securestackhub.blogspot.com${meta.legacyPath || `/${file.replace(/\.md$/, '')}`}`, title: meta.title, description: meta.description, contentHtml, category: meta.category || 'VPN Troubleshooting', publishedAt: meta.publishedAt || '', updatedAt: meta.updatedAt || '', wordCount: match[2].split(/\s+/).filter(Boolean).length });
}
const output = `export type StaticArticle = { slug: string; canonicalUrl: string; title: string; description: string; contentHtml: string; category: string; publishedAt: string; updatedAt: string; wordCount: number };\n\nexport const articles = ${JSON.stringify(articles, null, 2)} as const;\n`;
