...
export function getHead(path: string) {
  ...
  const title = article ? `${article.title} | SecureStack Hub` : path === "/de/" ? "VPN-Hilfe auf Deutsch | SecureStack Hub" : category ? "VPN Troubleshooting Guides | SecureStack Hub" : path === "/about/" ? "About SecureStack Hub | Editorial Standards" : feedback ? "Corrections & Feedback | SecureStack Hub" : "VPN Troubleshooting & Privacy Guides | SecureStack Hub";
  const description = article?.description || (path === "/de/" ? "Deutsche Anleitungen zu VPN-, DNS-, Windows-, Router- und Netzwerkproblemen für Deutschland, Österreich und die Schweiz." : category ? "Straightforward VPN troubleshooting guides for Wi-Fi, DNS, devices, and local networks." : feedback ? "Send SecureStack Hub a correction or website feedback through our public feedback page." : "Straightforward VPN troubleshooting and online privacy guides with clear steps, practical limits, and no hype.");
  return { title, description, canonical: `${origin}${path}`, article, locale: german ? "de" : "en" };
}
...
export function render(url: string) {
  ...
  const meta = `<title>${escape(head.title)}</title><meta name="description" content="${escape(head.description)}"><meta name="author" content="SecureStack Editorial"><meta name="robots" content="index,follow"><meta name="google-site-verification" content="dlCrSiMy1rU3QRhcMeH4FkiKGV_rt-N4SJB_1P8HJXI"><meta property="og:locale" content="${head.locale === "de" ? "de_DE" : "en_US"}"><link rel="icon" href="/favicon.svg" type="image/svg+xml"><link rel="canonical" href="${head.canonical}">${alternates}${jsonLd}`;
  return { html, head: meta, title: head.title, notFound: false, locale: head.locale };
}
